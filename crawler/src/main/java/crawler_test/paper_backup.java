...
public class paper_backup implements PageProcessor {
	//要抓取的学校名称
	public static final String Name = "北京工业大学";
	//用于支持断点续抓的缓存文件的路径，需提前新建文件夹，
	public static final String CacheFile = "/Users/sikai/Documents/workspace/crawler_test/Logs/test_demo/XBGD";
	//每个抓取页面间的时间间隔
    	...
    	if(!page.getUrl().regex(List_URL).match()){
    		...
    		if(page_url.contains("Periodical")||page_url.contains("Conference")||page_url.contains("NSTLQK")||page_url.contains("NSTLHY")){
    			...
    			if(belong_to_clg){
        			...
        			try {
            			conn  = ConnectionUtils.getConnection();
                        DBUtils.insertPaperXBGD(conn, ppr_sent);
                    } catch (SQLException | ClassNotFoundException e) {
                        e.printStackTrace();           
                    }finally {                      
                        if (conn != null) {
    ...
    public static void main(String[] args) {
    	...
    	Spider.create(new test_demo()).addUrl(final_url)
